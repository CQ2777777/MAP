                ...
                for pod_id in parked_pods:
                    if pod_id not in assigned_charging_pod_for_electric_veh:
                        ...
                        if eligible_evs:
                            ...
                            print(f"pod {pod_id}({source_pa}) is redistributed for EV {eligible_evs[0]}({target_pa})")
                            return
...
def get_state(vehicle_id=None, charging_pod_id=None):
    ...
    if vehicle_id is None:
        ...
        distance2D = traci.simulation.getDistance2D(veh_pos[0], veh_pos[1], pod_pos[0], pod_pos[1])
        p_edge = traci.vehicle.getRoadID(charging_pod_id)
...
def calculate_reward(vehicle_id, charging_pod_id, action):
    ...
    if action == "start_charging":
        ...
        if SOC_EV >= Max_charge_for_EVs or SOC_POD <= LOW_BATTERY_THRESHOLD:
            ...
            reward -= 15
        '''
    reward += (SOC_EV - 20) * 0.01
    reward -= (100 - SOC_POD) * 0.01
    return reward
...
def get_edge_density(edge_id):
    ...
    ev_count = sum(1 for veh in vehicles if traci.vehicle.getTypeID(veh) == "ElectricVehicle")
    return ev_count
                ...
                if vehicle_type == "ElectricVehicle":
                    try:
                        ...
                        if vehicle_id not in selected_targets:
                            if current_edge == "E0":
                                # 计算E1和E4的密度
                                density_e1 = get_edge_density("E1")
                                density_e4 = get_edge_density("E4")
                                next_edge = "E1" if density_e1 < density_e4 else "E4"

                            elif current_edge == "E4":
                                next_edge = "E5"  # 只有单一选择

                            elif current_edge == "E7":
                                next_edge = "E8"  # 只有单一选择

                            elif current_edge == "E8" or current_edge == "E9":
                                next_edge = "E10"  # 只有单一选择

                            elif current_edge == "E10":
                                next_edge = "E11"  # 只有单一选择

                            elif current_edge == "E11" or current_edge == "E2":
                                next_edge = "E3"  # 只有单一选择
                                ...
                                # 计算E2和E9的密度
                                density_e2 = get_edge_density("E2")
                                density_e9 = get_edge_density("E9")
                                next_edge = "E2" if density_e2 < density_e9 else "E9"

                            elif current_edge == "E5":
                                # 计算E6和E7的密度
                                density_e6 = get_edge_density("E6")
                                density_e7 = get_edge_density("E7")
                                next_edge = "E6" if density_e6 < density_e7 else "E7"

                            elif current_edge == "E6":
                                # 计算E2和E9的密度
                                density_e2 = get_edge_density("E2")
                                density_e9 = get_edge_density("E9")
                                next_edge = "E2" if density_e2 < density_e9 else "E9"
...
def predict(model_path, sumo_config_file):
    ...
    traci.start(["sumo-gui", "-c", sumo_config_file])
    traci.simulation.loadState("SAVE_state.xml.gz")
    proximity_map = optimized_proximity_map("test.net.xml")
    ...
    for step in range(simulation_duration):
        ...
        for vehicle_id in active_vehicles:
            ...
            if vehicle_type == "ElectricVehicle":
                try:
                    if traci.vehicle.getLanePosition(vehicle_id) > 430:
                        current_edge = traci.vehicle.getRoadID(vehicle_id)
                        # Once the vehicle moves to a new edge, allow it to choose again at the next intersection
                        if vehicle_id in selected_targets and current_edge == selected_targets[vehicle_id]:
                            del selected_targets[vehicle_id]
                    ...
                    if vehicle_id not in assigned_charging_pod_for_electric_veh and  (traci.vehicle.getLanePosition(vehicle_id) > 940 and traci.vehicle.getLanePosition(vehicle_id) < 950):
                        if current_edge == "E0":
                            density_e1 = get_edge_density("E1")
                            density_e4 = get_edge_density("E4")
                            next_edge = "E1" if density_e1 < density_e4 else "E4"
...
if __name__ == "__main__":
    #main()
    predict("model2_re/best_model_episode_29.pt", "loop2x2.sumocfg")